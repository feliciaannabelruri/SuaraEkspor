...
export default function ProductDetail() {
  ...
  const [activeTab, setActiveTab] = useState<'listings' | 'keywords' | 'promo'>('listings');
  const [copiedCode, setCopiedCode] = useState<string | null>(null);
  const [expandedListing, setExpandedListing] = useState<string | null>(null);
  const [shared, setShared] = useState(false);
  const [promoKit, setPromoKit] = useState<{ caption: string; hashtags: string[]; imageUrl: string } | null>(null);
  const [generatingPromo, setGeneratingPromo] = useState(false);
  const [promoError, setPromoError] = useState('');
  const [sharedPromo, setSharedPromo] = useState(false);
  ...
  async function handleGeneratePromoKit() {
    setGeneratingPromo(true);
    setPromoError('');
    try {
      const { data } = await apiClient.post(`/products/${id}/promo-kit`);
      setPromoKit(data.data);
    } catch (err: any) {
      setPromoError(err.response?.data?.error ?? 'Gagal membuat promo kit. Coba lagi.');
    } finally {
  ...
  async function handleSharePromoKit() {
    if (!promoKit) return;
    const text = `${promoKit.caption}\n\n${promoKit.hashtags.map(h => `#${h}`).join(' ')}`;
    try {
      const res = await fetch(promoKit.imageUrl);
      const blob = await res.blob();
      const file = new File([blob], `promo-${id}.jpg`, { type: blob.type || 'image/jpeg' });
    ...
    if (navigator.share) {
      ...
      setSharedPromo(true);
      setTimeout(() => setSharedPromo(false), 2000);
    }
  }
                ...
                <div className="p-6 border-b border-outline-variant/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-surface-container-lowest">
                  ...
                  <div className="flex gap-1.5 bg-surface-container p-1 rounded-lg border border-outline-variant/20 self-start sm:self-auto">
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${activeTab === 'promo' ? 'bg-primary text-on-primary shadow-sm' : 'text-on-surface-variant hover:text-on-surface'}`}
                    >
                      Promo Kit
                    </button>
                  </div>
                    ...
                    <div className="space-y-4">
                      {!promoKit && (
                        <div className="text-center py-8">
                          ...
                          >
                            {generatingPromo ? 'Membuat dengan AI...' : '+ Buat Promo Kit'}
                          </button>
                          {promoError && <p className="text-xs text-red-600 mt-3">{promoError}</p>}
                        </div>
                      )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex flex-wrap gap-2 pt-1">
                              <button onClick={handleDownloadPromoImage} className="flex items-center gap-1.5 bg-surface-container-high text-on-surface font-bold text-xs px-3 py-2 rounded-lg hover:bg-outline-variant transition-colors">
                                <span className="material-symbols-outlined text-[16px]">download</span> Unduh Gambar
                              </button>
                              <button onClick={handleSharePromoKit} className="flex items-center gap-1.5 bg-primary text-on-primary font-bold text-xs px-3 py-2 rounded-lg hover:opacity-90 transition-all">
                                <span className="material-symbols-outlined text-[16px]">{sharedPromo ? 'check' : 'share'}</span> {sharedPromo ? 'Tersalin' : 'Bagikan'}
                              </button>
                              <button
                                onClick={handleGeneratePromoKit}
                                disabled={generatingPromo}
                              ...
                              >
                                <span className="material-symbols-outlined text-[16px]">refresh</span> {generatingPromo ? 'Memproses...' : 'Buat Ulang'}
                              </button>
                            </div>
                          </div>
                        </div>
